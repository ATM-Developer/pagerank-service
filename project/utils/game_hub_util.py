...
def _retry(func, logger, times=3, reconnect=None):
    ...
    for i in range(times):
        try:
            ...
            last_exc = e
            logger.error('game hub call failed: {}'.format(e))
...
def _rank_uris_by_latest_block(rpc_urls, logger):
    ...
    for url in rpc_urls:
        try:
            ...
            logger.error('game hub rank failed for {}: {}'.format(url, e))
...
class GameHubReader:
            ...
            for url, number in ranked:
                ...
                try:
                    ...
                    self.logger.info('game hub selected uri: {} (block {}), connect took {:.2f}s'
                                     .format(url, number, time.time() - start))
                    self._w3 = w3
                    self._rebuild_contracts()
                    return
                except Exception as e:
                    self.logger.error('game hub uri failed: {}, {}'.format(url, e))
        raise ConnectionError('no reachable web3_provider_uri after {:.2f}s: {}'
                              .format(time.time() - start, rpc_urls))
    ...
    def fetch_instance_day(self, instance_address, since_date_key=None, on_date_done=None, known_calendar_dates=None):
        ...
        clamped to BOOST_START_DATE) through currentDateKey inclusive.

        dailyPointsFinalized(dateKey) - not dateKey order - is what actually
        gates "already finalized"; today's in-progress dateKey is still
        scanned so eligibility reflects live activity. Zero-point stakers are
        skipped since they can't contribute to any share.

        on_date_done(calendar_date, date_key, date_rows) fires per dateKey so
        the caller can persist progress incrementally; date_key lets the
        caller replace (not accumulate) that dateKey's rows on a re-fetch,
        since not-yet-finalized dateKeys return current state, not a delta.
        ...
        for date_key in range(start_date_key, current_date_key + 1):
            if date_key < 0:
                continue
            finalized = _retry(
            ...
            if cursor_still_consecutive and finalized:
        ...
        self.logger.info('instance {} fetch took {:.2f}s, dateKeys {}-{}, {} rows'
                         .format(instance_address, time.time() - start, start_date_key, current_date_key, len(rows)))
